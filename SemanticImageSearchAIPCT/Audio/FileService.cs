    ...
    public class FileService
    {
        public void SaveFloatArrayToFile(float[] floatData, string folderPath)
        {
            ...
            try
            {
                ...
                LoggingService.LogDebug($"Audio  saved successfully: {filePath}");
            }
            catch (Exception ex)
            {
                // Handle exceptions (e.g., access rights, disk not available)
                LoggingService.LogDebug("An error occurred: " + ex.Message);
            }
        }
        ...
        {
            //string jsonFilePath = @"C:\AIPCTemplate\SemanticImageSearchAIPCT\SemanticImageSearchAIPCT\assets\filters.json";
